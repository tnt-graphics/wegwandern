...
$check_parent_region_page_html = '<input type="hidden" id="template_region_type" value="" />';
...
<div class="region-hike-list-container">
	...
	<div class="ListSec">
		...
		$parent_regions_args = array(
			'taxonomy'         => array( 'wanderregionen' ),
			'hide_empty'       => false,
			'parent'           => 0,
		);

		$parent_regions = get_terms( $parent_regions_args );

		if ( !empty ( $parent_regions ) ) {
			$get_all_parent_regions = array_column($parent_regions, 'term_id');

			if ( !empty ( $wanderung_regionen ) ) {
				foreach ( $wanderung_regionen as $pr ) {
					if ( in_array( $pr, $get_all_parent_regions ) ) {
						$check_parent_region_page_html = '<input type="hidden" id="template_region_type" value="parent" />';
					}
				}
			}
		}

		echo $check_parent_region_page_html;
