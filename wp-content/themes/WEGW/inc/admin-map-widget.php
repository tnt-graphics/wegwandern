...
function wegwandern_map_custom_meta_box_markup() {
	...
	$recalculate_gpx                = get_field( 'recalculate_gpx_values', $current_hike_id );
	...
	<script type="text/javascript">
		...
		function calculateGPX(){
			...
			if( json_gpx_data !== undefined ) {
				console.log('Main');
				console.log(json_gpx_data);
				console.log('2');
				console.log(json_gpx_data.trk);
				console.log('3');
				console.log(json_gpx_data.trk.trkseg);
...
function wegw_gpx_json_data_update_on_import( $post_id ) {
	...
	if ( 'wanderung' == $posttype && ! empty( $gpx_file ) ) {
		$recalculate_gpx                = get_field( 'recalculate_gpx_values', $post_id );

		/* Check if recalculate option is `True` */
		if( $recalculate_gpx == 1 ) {
