...
function get_wanderung_sidebar_filter_query() {
	...
	if ( ! empty( $_POST['filtered_map_ids'] ) ) {
		$filtered_map_ids  = explode( ',', $_POST['filtered_map_ids'] );
		$wegw_filter_query['post__in'] = $filtered_map_ids;
	}
	...
	if ( $event_type == 'hover' ) {
		$allwanderung = '';
		$saved_queries =  get_transient( 'tourentportal_filter_query' );
		if( ! empty ( $saved_queries )){
			$saved_query = $saved_queries['hover']['query'];

			if($saved_query == $wegw_filter_query){
				$allwanderung = $saved_queries['hover']['posts'];
			}
		}

		$allwanderung = empty($allwanderung) ? get_posts( $wegw_filter_query ) : $allwanderung;

		$filter_data['hover'] = array(
			'posts' => $allwanderung,
			'query' => $wegw_filter_query
		);

		set_transient( 'tourentportal_filter_query', $filter_data, 30 * MINUTE_IN_SECONDS );
		...
		if( $event == 'total_hikes_filter' ){
			// pre(get_posts($wegw_filter_query));
			// pre($wegw_filter_query);
			$filtered_hike = get_transient( 'tourentportal_filter_query' );
			if( !empty( $filtered_hike ) && !empty( $filtered_hike['hover']['posts'] )){
				$all_hike   = $filtered_hike['hover']['posts'];
			}elseif( $page_type == 'region' ){
				$wegw_filter_query['posts_per_page'] = -1;
				$all_hike          = get_posts( $wegw_filter_query );
			}else{
				$all_hike = ! empty ( get_transient( 'all_wandrung_posts' ) ) ? get_transient( 'all_wandrung_posts' ) : get_all_wandrung();
			}
			$all_count         = count( $all_hike );
		}

		if ( $event == 'reset_hikes_filter' ) {	
			if( $page_type == 'region' ){
				$wegw_filter_query['posts_per_page'] = -1;
				$all_hike          = get_posts( $wegw_filter_query );
				set_transient( 'tourentportal_filter_query', '' , 30 * MINUTE_IN_SECONDS );
			}else{
				$all_hike = ! empty ( get_transient( 'all_wandrung_posts' ) ) ? get_transient( 'all_wandrung_posts' ) : get_all_wandrung();
				set_transient( 'tourentportal_filter_query', '' , 30 * MINUTE_IN_SECONDS );
			}
		...
		if ( ! empty( $allwanderung ) ) {
			//$posts_html       .= get_wanderung_all_hikes_query( $filtered_hike_ids );
			$posts_html       .= get_wanderung_all_hikes_for_cluster_markers( $page_type, $event, $event_type, $wegw_filter_query );
			$ar_posts[]        = $posts_html;
...
function wanderung_drag_map_hikes_load_more() {
	...
	$count                  = $_POST['count'];
	$wanderung_filter_query = ( isset( $_POST['wanderung_filter_query'] ) && $_POST['wanderung_filter_query'] != '' ) ? $_POST['wanderung_filter_query'] : '';
	$filtered_map_ids       = ( isset( $_POST['filtered_map_ids'] ) && $_POST['filtered_map_ids'] != '' ) ? $_POST['filtered_map_ids'] : '';
	$page_type              = $_POST['map_page'];
	$data_array             = generate_all_data_array();
...
function wanderung_drag_map_hikes_sort_query() {
	...
	$wanderungs = '';
	$saved_queries =  get_transient( 'sort_filter_query' );
	if( ! empty ( $saved_queries )){
		$saved_query = $saved_queries['sort']['query'];

		if($saved_query == $query){
			$wanderungs = $saved_queries['sort']['posts'];
		}
	}else{
		...
		set_transient( 'sort_filter_query', $filter_data, 30 * MINUTE_IN_SECONDS );
	}
	if( empty($wanderungs) ){
		$wanderungs = get_posts( $query );
		$filter_data['sort'] = array(
...
function wanderung_get_hike_details( $wanderung, $map_json = false ) {
	...
	$hike_aktivitat = get_the_terms( $wanderung->ID, 'aktivitat' );
 ...
 */
function get_wanderung_all_hikes_for_cluster_markers( $page_template = 'tourenportal_page', $event = 'reset_hikes_filter', $event_type = 'btnClick',  $wegw_filter_query = array() ) {
	
	$all_wandurung_posts = ! empty ( get_transient( 'all_wandrung_posts' ) ) ? get_transient( 'all_wandrung_posts' ) : get_all_wandrung();
	$total_hike_filters  = ! empty ( get_transient( 'tourentportal_filter_query' ) ) ?  get_transient( 'tourentportal_filter_query' ) : $all_wandurung_posts;

	if( $event == 'reset_hikes_filter' && $page_template == 'region'){
		$allwanderung   = get_posts($wegw_filter_query);
	}elseif( $event == 'reset_hikes_filter' &&  $event_type == 'btnClick'  && $page_template == 'tourenportal_page'){
		$allwanderung = $all_wandurung_posts;
	}elseif( $event == 'total_hikes_filter' && $page_template == 'region' ){
		$wegw_filter_query['posts_per_page'] = -1;
		$allwanderung   = get_posts($wegw_filter_query);

	}elseif( $event == 'total_hikes_filter' &&  $event_type == 'btnClick' && $page_template != 'region'){
		$allwanderung = !empty($total_hike_filters['hover']['posts']) ? $total_hike_filters['hover']['posts'] : $total_hike_filters;
	}
...
// add_action( 'wp_ajax_nopriv_wanderung_reset_filter_query', 'wanderung_reset_filter_query' );
// add_action( 'wp_ajax_wanderung_reset_filter_query', 'wanderung_reset_filter_query' );

// function wanderung_reset_filter_query(){
// 	//set_transient( 'tourentportal_filter_query', '', 15 * MINUTE_IN_SECONDS );
// 	wp_send_json_success();
// }


function get_all_wandrung(){

	$allwanderung = get_site_transient('all_wandrung_posts');
	...
	if( empty( $allwanderung ) ){
		...
		$allwanderung = get_posts( $arg );
		set_transient( 'all_wandrung_posts', $allwanderung, 30 * MINUTE_IN_SECONDS  );
	}
