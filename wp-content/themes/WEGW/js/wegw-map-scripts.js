    ...
    const app = {
        ...
        init: function () {
            ...
            var zoom = 5;
                ...
                if($('.single-wander-wrappe-json').length > 0 || $('.filter-btn.region-filter').length > 0){
                    app._initLayer(swisstopo_layer);
                    mapDesk = new ol.Map({
        ...
        _isCSSLoaded: function(url){// Function to check if a CSS file is already loaded
            return document.querySelector('link[href="' + url + '"]') !== null;
        },
        _isJSLoaded: function(url) {// Function to check if a JavaScript file is already loaded
            return document.querySelector('script[src="' + url + '"]') !== null;
        },
        _insertCSS: function(url) {

            // Function to insert CSS dynamically if not already loaded
            if (!app._isCSSLoaded(url)) {
        ...
        _insertJS: function(url, callback, ele) {// Function to insert JavaScript dynamically if not already loaded
            if (!app._isJSLoaded(url)) {
              ...
              //  callback();
              app._renderOpenPopupMap(ele);
            }
        },
        _checkAllScriptsLoaded: function(ele, jsURLs) {
            loadedScripts++;
            if (loadedScripts === jsURLs.length) {
                app._renderOpenPopupMap(ele);
            }
        },
        _renderOpenPopupMap: function(ele){
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                    if (response) {
                        if(jQuery('.map_region').length > 0){// popup for region page
                        ...
                        }else{//popup other than region page and touernportal page
                            ...
                            jQuery('#weg-map-popup #mapOptions').html("");
                            ...
                            if( parseJsonData.gpx_file_name === "" ) { 
                               ...
                               // document.getElementById('mapOptions').innerHTML = "<h2>GPX file is missing.</h2>";
                               jQuery('#weg-map-popup #mapOptions').html("<h2>GPX file is missing.</h2>");
                                app._layerFunction(map, 'weg-map-popup-inner-wrapper', 'weg-map-popup', null, hikeID);
                            } else {
                                json_gpx_data = JSON.parse( JSON.parse(response).json_gpx_data);
                                gpx_file = JSON.parse(response).gpx_file_name;
                                app._gpxData(json_gpx_data, 'weg-map-popup-inner-wrapper', 'weg-map-popup', hikeID);
                            }
        ...
        _openPopupMap: function(ele) {
            //ol lib css
			var cssURLs = [
				window.location.origin+ "/wp-content/themes/WEGW/lib/ol@v7.1.0/ol.css",
				window.location.origin+ "/wp-content/themes/WEGW/lib/ol-ext/ol-ext.css"
			];
			//ol lib js
			var jsURLs = [
				window.location.origin+ "/wp-content/themes/WEGW/lib/ol@v7.1.0/ol.js", // OpenLayers library
				window.location.origin+ "/wp-content/themes/WEGW/lib/ol-ext/proj4.js",
				window.location.origin+ "/wp-content/themes/WEGW/lib/ol-ext/ol-ext.min.js"
			];
            ...
            jsURLs.forEach(function(url) {
                app._insertJS(url, app._checkAllScriptsLoaded(ele, jsURLs), ele);
            });
            ...
            // if( typeof json_gpx_data !== 'undefined') {
                if ( json_gpx_data != false ) {
                   ...
                   // var popupWrappercontent =document.getElementById('detailPgPopupContent');
                    if($('.map_region').length > 0){// for region page
                    ...
                    if($('.map_region').length > 0){// for region page with popup
                    ...
                    }else{//for other pages with popup
                        if(jQuery('#weg-map-popup-inner-wrapper').length > 0){
                            app._initMap([900000, 5900000], 12, ol.control.defaults.defaults().extend([new ol.control.FullScreen()]), 'weg-map-popup-inner-wrapper'); 
                            app._layerFunction(map, 'weg-map-popup-inner-wrapper','weg-map-popup-inner-wrapper', null, hikeID);
                            jQuery('#weg-map-popup #mapOptions').html("<h2>GPX file is missing.</h2>");
                        }
                    }
        ...
        _elevationPoint: function (map, gpx_elevation_points, gpx_trackpoints,  max_altitude, min_altitude, parentDiv) {
            ...
            if (gpx_elevation_points !== undefined) {
                ...
                let selector;
                if(jQuery('.map_region').length > 0){// elevation for region page
                    selector = document.querySelector('#'+ parentDiv+' .options');
                }else{// elevation for other page
                    selector = document.querySelector(".options");
                }
                    ...
                    if (e.type == "over") {
                        ...
                        for(i=0; i< gpx_trackpoints.length; i++){
                            if( parseFloat(gpx_trackpoints[i].ele) === e.coord[2]){
                                if(jQuery('.map_region').length > 0){// drawpoint for region page
                            ...
                            if(jQuery('.map_region').length > 0){//  for region page
        ...
        _placeCoord: function(map, mapContainer){
           ...
           $('.filter_reset').on('click', function(e){
            ...
            $('#wegw_map_filter_btn').on('click', function(){
                clusterSource.getSource().clear();
