...
class FrmRegModerationController {
	...
	public static function needs_moderation( $settings ) {
		return ! empty( $settings['reg_moderate'] );
	}
	...
	private static function log_user_in_without_password( $user_id ) {
		if ( ! is_user_logged_in() ) {
			$settings = FrmRegAppHelper::get_registration_settings_for_user( $user_id );
	...
	*/
	...
	private static function get_redirect_for_activated_user( $user_id, $moderation ) {
		...
		$form_settings         = FrmRegAppHelper::get_registration_settings_for_user( $user_id );
		$on_email_confirmation = isset( $form_settings['on_email_confirmation'] ) ? $form_settings['on_email_confirmation'] : 'page';

		if ( 'url' === $on_email_confirmation ) {
			if ( ! empty( $form_settings['reg_redirect_url'] ) ) {
				$redirect = self::filter_redirect_url( $form_settings['reg_redirect_url'], $user_id );
			}
		} elseif ( ! empty( $form_settings['reg_redirect'] ) ) {
		...
		if ( self::is_moderation_complete( $moderation ) ) {
			if ( 'page' === $on_email_confirmation ) {
				$params   = array(
					'frm_message' => 'complete',
					'user'        => $user_id,
				);
	...
	/**
	 * Returns filtered redirect to url.
	 *
	 * @since 2.13
	 ...
	 * @return string
	 */
	private static function filter_redirect_url( $redirect_url, $user_id ) {
		$form         = FrmRegAppHelper::get_form_for_user( $user_id );
		$entry_id     = FrmRegEntry::get_entry_for_user( get_user_by( 'ID', $user_id ) );
		$redirect_url = apply_filters( 'frm_content', $redirect_url, $form->id, $entry_id );

		return do_shortcode( $redirect_url );
	}
	...
	private static function after_activate_user( $moderation_type, $user ) {
		...
		$settings = FrmRegAppHelper::get_registration_settings_for_user( $user->ID );
	...
	public static function print_activation_messages( $content ) {
		...
		if ( isset( $_GET['frm_message'] ) && $_GET['frm_message'] == 'complete'
			...
			} elseif ( ! empty( $settings['reg_password'] ) ) {
