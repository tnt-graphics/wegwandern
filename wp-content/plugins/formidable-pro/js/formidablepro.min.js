...
phoneInput.addEventListener("blur",function(){phoneInput.value=intlTelInputInstance.getNumber()});intlPhoneInputs[phoneInput.id]=intlTelInputInstance})}return{init:function(){maybeAddPolyfills();maybeAddIntlTelInput(document.querySelectorAll(".frm-intl-tel-input"));addEventListener("pageshow",maybeTriggerCalc);jQuery(document).on("frmFormComplete",afterFormSubmitted);jQuery(document).on("frmPageChanged",afterPageChanged);jQuery(document).on("frmAfterAddRow frmAfterRemoveRow",calcProductsTotal);jQuery(document).on("click",
